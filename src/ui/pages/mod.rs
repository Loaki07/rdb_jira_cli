...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            ...
            let status_col = get_column_string(
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let epic =
            db_state.epics.get(&self.epic_id).ok_or_else(
                || anyhow!("could not find epic!"),
            )?;
        ...
        println!(
            "{} | {} | {} | {}",
            id_col, name_col, desc_col, status_col
        );
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let story = db_state
            ...
            .get(&self.story_id)
            .ok_or_else(|| {
                anyhow!("could not find story!")
            })?;

        println!("------------------------------ STORY ------------------------------");
        println!("  id  |     name     |         description         |    status    ");
        let id_col = get_column_string(
...
mod tests {
    ...
    mod home_page {
        ...
        fn handle_input_should_return_the_correct_actions()
        {
            ...
            assert_eq!(
                page.handle_input(invalid_epic_id).unwrap(),
                None
            );
            assert_eq!(
                page.handle_input(junk_input).unwrap(),
                None
            );
            assert_eq!(
                page.handle_input(
    ...
    mod epic_detail_page {
        ...
        fn handle_input_should_return_the_correct_actions()
        {
            ...
            assert_eq!(
                page.handle_input(&story_id.to_string())
                    .unwrap(),
            ...
            assert_eq!(
                page.handle_input(invalid_story_id)
                    .unwrap(),
                None
            );
            assert_eq!(
    ...
    mod story_detail_page {
        ...
        fn handle_input_should_return_the_correct_actions()
        {
            ...
            assert_eq!(
                page.handle_input(some_number).unwrap(),
                None
            );
            assert_eq!(
                page.handle_input(junk_input).unwrap(),
